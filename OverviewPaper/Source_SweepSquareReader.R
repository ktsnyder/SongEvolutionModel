...
Stitcher <- function(Direct, NewFolder="Stitched"){
  ...
  Usable2 <- strsplit(Usable,"[[:alpha:]]")
  Usable2 <- as.numeric(sapply(1:length(Usable2), function(x) Usable2[[x]][1]))
  datas <- unique(Usable2)
  dir.create(file.path(Direct,NewFolder))
  for(j in seq_along(datas)){
    readme <- which(Usable2==datas[j])
    data <- as.list(1:length(readme))
    for(i in seq_along(readme)){
      data[[i]] <- read.table(file.path(Direct,Usable[readme[i]]), header=FALSE, stringsAsFactors = FALSE, fileEncoding = "UTF-8-BOM")
      #data[[i]]<- gsub("﻿", "", unlist(Data[[i]]))
    }
    
    df <- data.frame(matrix(unlist(data), ncol=length(readme)))
...
GetUsableFiles <- function(Direct){
  ...
  Usable <- Usable[which(Usable %in% c("semp", "csv")==FALSE)]
...
QuadPlots <- function(Labels, Order, Overview=FALSE, Color=c("azure3","white","azure2"),
  ...
  SweepPlots(Data$LBoxer[,Order[subset]], "Learning Threshold", Labels[[1]],Labels[[2]],
             Labels[[3]], Labels[[4]], cuts, Top=20, Color=Color, BoxWex=BoxWex, Letter='B', End=End)
