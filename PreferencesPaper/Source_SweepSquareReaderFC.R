...
Stitcher <- function(Direct, NewFolder="Stitched"){
  ...
  for(j in seq_along(datas)){
    ...
    Names <- gsub("[.]csv","",Usable[c(1,3,4,6)])
...
LoadData <- function(Direct, NSims=4000, Freq=200){
  ...
  which(!(1:168 %in% Usable))
  Leng <- length(Usable)*2
  interval <- (NSims/Freq+1)
  LengAll <- length(Usable)*interval
  FinalData <- data.frame(PRepPref=numeric(Leng), PMatchPref=numeric(Leng),
                          PSocPref=numeric(Leng), PFreqPref=numeric(Leng),
                          Mstrat=character(Leng), Rare=logical(Leng),
                          Preference=character(Leng), PLrnStrtgy=character(Leng),
  ...
  for(i in seq_along(Usable)){
    ...
    if(P$Consen){
      if(P$ConsenS == "AllNone"){
        Strat <- "AllNone"  
      }else if(P$ConsenS == "Conform"){
        Strat <- "Conform"
      }else{
        Strat <- "Percen"
      }
    ...
    Param <- c(P$RepPref, P$MatPref, P$SocPref, P$FreqPref,
               P$MStrat,  P$Rare, Pref,
               Strat, P$RSize0,
...
QuadPlots <- function(Labels, Order, Overview=FALSE, Color=c("azure3","white","azure2"),
                      ...
                      Match=TRUE, End=TRUE, LinePush=-3){
  ...
  SweepPlots(Data$RBoxer[,Order[subset]], "Repertoire Size", Labels[[1]],Labels[[2]],
             Labels[[3]], Labels[[4]], cuts, Color=Color, BoxWex=BoxWex, Letter='A', End=End)
  if(Overview){
    ExtraLables(length(subset), max(Data$RBoxer))
  }
  SweepPlots(Data$LBoxer[,Order[subset]], "Learning Threshold", Labels[[1]],Labels[[2]],
             Labels[[3]], Labels[[4]], cuts, Top=7, Color=Color, BoxWex=BoxWex, Letter='B', End=End)
  SweepPlots(Data$ABoxer[,Order[subset]], "Accuracy", Labels[[1]],Labels[[2]],
             Labels[[3]], Labels[[4]], cuts,Top=1, Color=Color, BoxWex=BoxWex, Letter='C', End=End)
  if(Match){
    SweepPlots(Data$MBoxer[,Order[subset]], "Matching", Labels[[1]],Labels[[2]],
               Labels[[3]], Labels[[4]], cuts, Top=1, Color=Color, BoxWex=BoxWex, Letter='D', End=End)
    if(Overview){
...
getAverageSong <- function(P, data, rep=50, freq=20, ave=TRUE){
  Interval <- P$nSim/freq+1
  BaseLocation <- 0:(rep-1)*(P$MaxRSize*Interval)
  data <- t(as.matrix(data))
  return(lapply(0:(Interval-1), function(i)
    matrix(sapply(1:P$MaxRSize,
                  function(j) 
                    if(ave){mean(data[BaseLocation+j+P$MaxRSize*i])
                    }else{
                      sd(data[BaseLocation+j+P$MaxRSize*i])
                    }),
           ncol=1))
  )
}
...
TableData <- function(path, N){
  df <- data.frame(corner=numeric(N), center=numeric(N),
                   centersd=numeric(N), edge=numeric(N),
                   node=numeric(N), farcorner=numeric(N),
                   WithDial=numeric(N), CrossDial=numeric(N))
  for(omg in 1:N){
    ...
    height <- P$R/Div[1]
    width <- P$C/Div[2]
    corner <- Averages[[21]][1]
    center <- Averages[[21]][P$R*(floor(width/2)) + floor(height/2)]
      centersd <- Stdev[[21]][P$R*(floor(width/2)) + floor(height/2)]
    edge <- Averages[[21]][P$R*(width-1) + floor(height/2)]
    node <- Averages[[21]][P$R*(width-1)+height]
    farcorner <- Averages[[21]][P$C*P$R]
    CrossDial <- corner-farcorner
    WithDial <- center-edge
    df[omg,] <- c(corner, center, centersd, edge, node, farcorner, WithDial, CrossDial)
  }
  return(df)
}
