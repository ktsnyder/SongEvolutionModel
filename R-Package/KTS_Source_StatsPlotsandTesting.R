...
#' @keywords stats-plotting
#' @export
TraitPlot <- function(trait, xlab="Time Steps", ylab, timeInterval = NULL){
  #processing
  if (is.null(timeInterval)) {
    trait=trait
    xlims=c(0, ncol(trait))
    t0=0
  } else {
    trait=trait[,timeInterval[1]:timeInterval[2]]
    xlims=timeInterval
    t0=timeInterval[1]-1
  }
  ...
  All <- 1:ncol(trait)+t0
  First <- 1:(ncol(trait)-1)+t0
  Second <- 2:ncol(trait)+t0
  ...
  segments(First, Means[First-t0], Second, Means[Second-t0])
}
...
#' @keywords stats-plotting
#' @export
SongPlot <- function(P, songs, thin = 10, male=TRUE, timeInterval=NULL, SyllInterval=NULL, YouWantItDarker=1){
  if(male){
    Type <- "Male"
  }else{
    Type <- "Female"
  ...
  if (is.null(SyllInterval)) {
    SyllInterval= c(P$MaxRSize,1)
  } else {
    SyllInterval = c(max(SyllInterval),min(SyllInterval))
  }

  if (is.null(timeInterval)) {
    timeInterval = c(1,ncol(songs))
    t0=0
  } else {
    t0=timeInterval[1]-1
  }
  songs <- songs[,seq(timeInterval[1],timeInterval[2],by=thin)]/P$numBirds
  plot(0, type="n",ylim=SyllInterval, xlim=c(timeInterval[1]/thin, timeInterval[2]/thin),
       xlab=paste0("TimeStep",thinlab), ylab=paste(Type, "Syllables"), font.lab=2,
       cex.lab=1.2)

  for(i in timeInterval[1]:(timeInterval[2]/thin)){
    for(j in SyllInterval[2]:SyllInterval[1]){
      rect(i-1, j-1, i, j, col=rgb(0, 0, 0, songs[j,i-t0]*YouWantItDarker), border=NA)
    }
  }
}
