...
# Edited by Kate Snyder 6/6/2022 - GenerateChicks
...
#' @keywords birth
#' @export
GenerateChicks <- function(P, fatherInd, territorialMales, vacancy, Stress=FALSE, StressAccuracyHit = NULL){
  ...
  # Adjust chick accuracy based on Stress - KTS
  if (Stress) {
    Males$Acc <- Males$Acc*StressAccuracyHit
  }

  #Song inherited from father based on chick accuracy
  if(P$Vert){
    InherSong <- VerticalSongLearning(P = P, templates =  MSongs, chicks =  Males)  # can adjust accuracy outside of this in "Males"
  }else{
