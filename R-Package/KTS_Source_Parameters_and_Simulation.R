...
#
#Last edited by Kate Snyder 6/6/2022 - Basic Simulation; add ability to impose stress for a certain duration
...
DefineParameters <- function(Rows=20, Cols=20, Steps=1,
                             InitialSylRepSize=38, PrcntSylOverhang=.6, MaxSylRepSize=90,
                             InitialAccuracy=.7, InherAccuracyNoise=0,  AccuracyLimits=c(0,1),
                             MaxAge=10, InitialLearningThreshold=0.9, InherLearningNoise=0, LearningLimits=c(0,1),
                             InitialChancetoInvent=0, InherChancetoInventNoise=0, ChancetoInventLimits=c(0,1),
                             InitialChancetoForget=0, InherChancetoForgetNoise=0, ChancetoForgetLimits=c(0,1),
                             ListeningThreshold=0.999, FatherListeningThreshold=.999, MinLearnedSyls=5,
                             EncounterSuccess=.95, LearningPenalty=0, AgeDeath=TRUE,
                             PrcntRandomDeath=.1, DeathThreshold=1, ChickSurvival=0.6,
                             LocalBreed=FALSE, LocalTutor=FALSE, LearnerStrategy="AddForget",
                             ...
                             RepSizePrefer=0, LogScale=TRUE, MatchPrefer=0, UniformMatch=TRUE, MatchScale=1,
                             Dialects=1, MaleDialects="None", FemaleEvolve=FALSE, ChooseMate=FALSE,
                             SaveMatch=NA, SaveAccuracy=T, SaveLearningThreshold=NA, SaveChancetoInvent=NA, SaveChancetoForget=NA,
                             SaveNames=T, SaveAge=T, SaveMaleSong=T, SaveFemaleSong=FALSE,
                             numSim=1000, Seed=NA){
  numBirds <- Rows*Cols
  if(AgeDeath){
...
## DefineParameters original defaults:
# Rows=20, Cols=20, Steps=1,
...
#' @export
SEMSimulation <- function(P, type='Basic', folderName=NA, save=TRUE, return=FALSE, verbose=TRUE, AccuracyInsultSims = NULL, StressAccuracyHit = NULL, ...){
  Time <- proc.time()
  MiscArgs <- list(...)
  ...
  #Run Simulations
  if(type == 'Basic'){
    Data <- BasicSimulation(P, MiscArgs$freq, AccuracyInsultSims, StressAccuracyHit)
  }else if(type == 'Light'){
...
#' @keywords read-write-run
#' @export
BasicSimulation <- function(P, freq=1, AccuracyInsultSims = NULL, StressAccuracyHit = NULL){
  if(is.null(freq)){
    freq <- 1
  }
  ...
  PrevSimStress <- FALSE # kts added
  #Run the simulation
  Counter <- 2
  for(i in 2:(P$nSim+1)){

    if (PrevSimStress) { # kts added
      Population$Males$Acc[which(Population$Males$Age == 0)] <- Population$Males$Acc[which(Population$Males$Age == 0)]*StressAccuracyHit^-1   # restore Chicks' accuracies to what they were originally inherited at, so that they pass on their accuracy based on genes, not stress experience   - note: if birds can learn for a duration longer than 1 sim, they'll have higher accuracy their second year
    }

    if (i %in% AccuracyInsultSims) {   # kts added
      Population <- BirthDeathCycle(P, Population, Stress = TRUE, StressAccuracyHit = StressAccuracyHit)  # kts added
      PrevSimStress <- TRUE
    } else {
